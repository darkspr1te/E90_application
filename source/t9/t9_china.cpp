...
#define TS_KEY_COUNT        8

char TS_SYmb[]={"qwertyuiopasdfghjklzxcvbnm"};
...
QChar chn_TsPress(int nKey)
{
	...
	else if (nKey == UIABC_DOWN)
	{
			g_PageIndex++;
	}
	else if (nKey >= UIABC_Q && nKey <= UIABC_M)
	{
		if(g_nEditStatus == T9_Start)
		{
			g_nEditStatus = T9_Edit;
		...
		if (g_KeyBufIdx < 8 && g_nEditStatus == T9_Edit)
		{
			g_KeyBuffer[g_KeyBufIdx++] = nKey - UIABC_Q;
			if (!GetAlphaStr())
				g_KeyBufIdx--;
			else
				g_SubIndex = 0;
	...
	else if (nKey >= UIABC_PY1 && nKey <= UIABC_PY9)
	{
        if (g_KeyBufIdx && g_nEditStatus == T9_Select)
		{
			QString str = QWidget::tr(GetChineseStr());

			chRet = str[g_PageIndex * 9 + nKey - UIABC_PY1];
			if (chRet != 0)
				chn_Init();
		}
	}
