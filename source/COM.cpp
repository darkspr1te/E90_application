...
#define TCP_RECV_CMD_TIMEOUT 20  
...
int   nRealSpaceTime = 10; 


//
//
//////////////////////////////////////////////////////////////////////////
void FileReadErrtoRest(void)
{
	int fd;
	int nret;
	...
	fd = open(FLASH_PART_FILENAME, O_RDWR | O_CREAT);
	if (fd == INVALID_HANDLE_VALUE)
		nret= FALSE;
	FD_CLOSE(fd);
...
//	wPortNum = dbComm_wPortNum;
	...
	comm_sendtime();
    xprintf("%s", "comm_tcpclient_connect...\r\n");	
...
//		client_addr.sin_addr.s_addr = htonl(dbComm_ipPcBackground);
		...
		if (!dbSetupTotal.setSystem.szHostByName[0])
		{
			 client_addr.sin_addr.s_addr = htonl(dbComm_ipPcBackground);
			 client_addr.sin_port = htons(wPortNum);
		}
...
int comm_sockrecv(HFILE hSock, void *pBuffer, int len, int nTimeout)
{
	...
	else
	{
		...
		while ((len > 0) && GetMainTickCount() < nETime)
		{
			...
			nCurrentReadSize = recv(Client_Sock, pBufIdx, nCurrentReadSize, 0);
...
int comm_socksend(HFILE hSock, void *pBuffer, int len)
{
	...
	//		comm_uartsend(0, &nCurrentWrittenSize, sizeof(nCurrentWrittenSize));
...
static volatile int __tcpclient_connect_status = 0;
		...
		(!g_bDisableDevice || g_bDisableDevice == DISABLE_MODE_TCP)) //TCPIP
	{
		...
		for (int i=0; i<MAX_SOCKS; i++)
		{
			if (g_hClientSocks[i] == INVALID_SOCK_HANDLE && dbLicense.byTcpipMode == TCPIP_SERVER)  
				continue;                                  
			if (arm_comm_tcp_get_cmd(g_hClientSocks[i], &vCMD))
			{
				comm_sendtime();
				xprintf("deal with tcpip arm_data (cmd=%d)\n", vCMD.Command);
				comm_sub_mode = i;
				Com_CommanProc(&vCMD);
			...
			{
				memcpy(&sCMD, &vCMD, STAFF_SIZE);
				comm_crypt(&sCMD, STAFF_SIZE, COMM_PWD_TCPIP);
				...
				if (comm_check_cmd(&sCMD))
				{
					...
					comm_sub_mode = i;
					comm_sendtime();
                    xprintf("deal with tcpip data (cmd=%d)\n", sCMD.data);
					
//					if(g_dwWebserverIP == 0x100007F){//Webserver
						hcCommanProc(sCMD.data);
//					}//Setting Prog
// 					else if((sCMD.data >= CMD_DOWN_DOWNLOADER_WRITE && sCMD.data <= CMD_TEST_GET_IMAGE) || sCMD.data == CMD_INTERNAL_CHECK_PWD){
// 						hcCommanProc_Set(sCMD.data);
// 					}
					g_hClientSocksInfo[i] = uiTimeGetTickCount();
					comm_mode = COMM_MODE_NONE;
...
// 				}
	...
	if ((dbLicense.byTcpipMode == TCPIP_CLIENT) && bSendAckTimeout && (uiTimeGetTickCount() - dwSendAckRealTime >= 120 * 1000)) //此处定时值必须大于nRealSpaceTime 3倍以上。 
	{
        		comm_sendtime();
...
//		    DM9000_Reset();
	...
	/*	
		...
		else if (DbGLogCount() != 0 )
		{
			ALOG_INFO gLog;  
			DbGLogGet(&gLog, DbGLogGetReadPos(), 1);  
			if (uiEventSend_GLOG(&gLog) && dbSetupTotal.setSystem.nRealSpaceTime != 0)
                 			nRealSpaceTime = dbSetupTotal.setSystem.nRealSpaceTime;
			else 
				nRealSpaceTime = 10;
		}
		else if (DbSLogCount() != 0 )
		{
			MLOG_INFO mLog;  
			DbSLogGet(&mLog, DbSLogGetReadPos(), 1);  
			if (uiEventSend_SLOG(&mLog) && dbSetupTotal.setSystem.nRealSpaceTime != 0)
                 			nRealSpaceTime = dbSetupTotal.setSystem.nRealSpaceTime;
			else 
				nRealSpaceTime = 10;
		}
		else if (ComClientFpDataUpLoad())
		{
		   	 nRealSpaceTime = 10;
		}
		else 
		{
			...
			if (uiEventSend_HEART() && dbSetupTotal.setSystem.nRealSpaceTime != 0)
				 nRealSpaceTime = dbSetupTotal.setSystem.nRealSpaceTime;
			else nRealSpaceTime = 10;
		}
	*/	
...
BOOL ComSetEnrollDataFP( CMDPKT* pCMD )
{
	...
	DbUserInfoFlagManagerSet(&gUserInfoTemp, nUserPriv);

	if (pUserInfo && !DbUserInfoEnroll(&gUserInfoTemp, BACKUP_ALL, NULL))
	{
		Com_SendExeResult( EXE_RES_FALSE, 0 );
		return FALSE;
	}
...
BOOL ComGlogGetByTime ( CMDPKT* pCMD )
{
	...
	else
	{
		...
		xprintf("from=%d Count=%d fromsecond=%d tosecond=%d \r\n", LogIndex[0],SendCount,LogIndex[1], LogIndex[2]);
		...
		DWORD _convGlog[] = {0xCF, 0xCF, 1, 0xCF, 3, 0xCF, 2, 0xCF, 0xCF, 0xCF};
		for(i = 0; i < SendCount; i++){				
			xprintf("send nID=%d dwSeconds=%d byAction=%d \r\n",  pLogs[i].nID, pLogs[i].dwSeconds, pLogs[i].byAction);
			pLogs_ARM[i].dwSeconds = pLogs[i].dwSeconds;
			pLogs_ARM[i].wId = (DWORD)pLogs[i].nID;
			pLogs_ARM[i].byAction = _convGlog[pLogs[i].byAction];
		}
...
BOOL ComModifyPrivilege(CMDPKT *vCMD)
{
	...
	BYTE nUserPriv = (BYTE)comm_OCX2Machine_UserPriv(vCMD->InParam);
	if(nUserPriv == 0){
		Com_SendExeResult( EXE_RES_FALSE, 0 );
		return FALSE;
	}

	DbUserInfoSetPrivilege(ID, nUserPriv);
	Com_SendExeResult( EXE_RES_TRUE, 0 );
	return nRet;
}
...
{
	char szPassSerialBuffer[32];
	char szPassSerialBuffer1[32];
	char szPassSerialBuffer2[32];
	char szPassSerialNumber[32];
	int szBuffer, szBuffer1, szBuffer2;
	int szPassSerialA = 0xefd59;
	int szPassSerialB = 0x132cc;
	int i;

	memset(&szPassSerialBuffer[0], 0, sizeof(szPassSerialBuffer));
	memset(&szPassSerialBuffer1[0], 0, sizeof(szPassSerialBuffer1));
	memset(&szPassSerialBuffer2[0], 0, sizeof(szPassSerialBuffer2));
	memcpy(&szPassSerialBuffer[0],&dbLicense.szSerialNumber[0], sizeof(dbLicense.szSerialNumber));

	for(i=0; i<6; i++)szPassSerialBuffer1[i]=szPassSerialBuffer[i];
	for(i=0; i<5; i++)szPassSerialBuffer2[i]=szPassSerialBuffer[i+6];
	szBuffer1= atoi(szPassSerialBuffer1);
	szBuffer2= atoi(szPassSerialBuffer2);

	szBuffer = (( szBuffer1 ^ szPassSerialA) | szBuffer2 ) + (( szBuffer2 | szPassSerialB) ^ szBuffer1 );
...
BOOL ComUserNameGet(CMDPKT *vCMD)
{
	...
	Com_SendExeResult( EXE_RES_TRUE, 0 );
	//char szUserName[10];
	//strcpy(szUserName, (char*)STR2ASCII(UNICODE2STRING(pUInfo->NAME,10)));
	//Com_SendData((void *)szUserName, 10);
    char szUserName[24];
...
BOOL ComUserNameSet(void)
{
	...
	//STRING2UNICODE(packed_name_info.NameA, gUserInfoTemp.NAME, USERNAME_LENGTH);
	memcpy(&gUserInfoTemp, pUInfo, sizeof(USER_INFO));
...
BOOL ComBellTimeGet(void)
{
	...
	for(i = 0; i < 8; i++){
		byBellTime[4 * i] = (BYTE)dbSetupTotal.setTimezone.tzBell[i].RANGE.bEndHour;
		byBellTime[4 * i + 1] = dbSetupTotal.setTimezone.tzBell[i].RANGE.bStartHour;
		byBellTime[4 * i + 2] = dbSetupTotal.setTimezone.tzBell[i].RANGE.bStartMinute;
		byBellTime[4 * i + 3] = 0;
	}

	Com_SendData((void *)&byBellTime[0], sizeof(byBellTime));
	Com_SendExeResult( EXE_RES_TRUE, ITEM_CNT(byBellTime));
	return TRUE;
}
...
//	dbSetupTotal.setNoMenu.nBellCount = vCMD->InParam;
	ret = Com_RecData( (void*)&byBellTime, sizeof(byBellTime) );
	if ( ret != 1 )
	{
		Com_SendExeResult( EXE_RES_FALSE, 0 );
		return FALSE;
	...
	for(i = 0; i < 8; i++){
		dbSetupTotal.setTimezone.tzBell[i].RANGE.bEndHour = byBellTime[4 * i];
		dbSetupTotal.setTimezone.tzBell[i].RANGE.bStartHour = byBellTime[4 * i + 1];
		dbSetupTotal.setTimezone.tzBell[i].RANGE.bStartMinute = byBellTime[4 * i + 2];
	}
	DbSetupTotalWrite();
//	uiRtcBellTimeInit();
...
{
	int nSLogReadCnt = DbSLogGetReadPos();
	DbSLogSetReadPos((nSLogReadCnt+1) % dbLicense.nSlogMaxCount);
	Com_SendExeResult( EXE_RES_TRUE, 0 );
	return TRUE;
}
		...
		{  
			...
			while((uiTimeGetTickCount()-dwTime)<5000) //有数据时 InParam == 1 Length == enrollid
			{
				...
				{					
					...
					//bResult = uiEnrollUserInfo2(&gUserInfoTemp);
					//if (bResult)
					{							
						xprintf("gUserInfoTemp.ID=%d \r\n", gUserInfoTemp.ID); 
						xprintf("gUserInfoTemp.FP=%d \r\n", gUserInfoTemp.FP); 
						xprintf("gUserInfoTemp.NAME=%s \r\n", gUserInfoTemp.NAME); 						
						DbUserInfoEnroll(&gUserInfoTemp, UserData.dwBackupNumber, pData, TRUE, FALSE);
						
						//if(DbUserInfoEnroll(&gUserInfoTemp, UserData.dwBackupNumber, &UserData.FingerData[8])==0)
						//	DbUserInfoDelete(gUserInfoTemp.ID, 1<<UserData.dwBackupNumber);
		
					}
					xprintf("gUserInfoTemp.FLAG.members.byMgr=%d \r\n", gUserInfoTemp.FLAG.members.byMgr); 
			...
			comm_mode = COMM_MODE_NONE;
			comm_sub_mode = -1;
			xprintf("%s \r\n", "EVENT_VERIFY_GET_DATA ERR"); 
			nRet=UIENROLL_RET_FALSE;
			return	nRet;			
		lexit:	
			xprintf("ComClientGetVerifydata exit \r\n"); 
			g_hClientSocksInfo[i] = uiTimeGetTickCount();
			comm_mode = COMM_MODE_NONE;
			comm_sub_mode = -1;
...
/*	msgBox.SetIcon(QString(GUI_RESOURCE_PATH) + QString("icon-question.png"));
	...
	if (uiLcdMessageBox(UI_MSG_QUESTION, UISTR_PRESS_VERIFY_GET_DATA, UITIME_OUT))
	{
        int  nRet;
//	if(QApplication::focusWidget()==(QWidget*)pMessageBoxbtnOK)
//	{		
		nRet=ComClientGetVerifydata(nID);
		if(nRet==UIENROLL_RET_DUPLICATE)
		{
			uiLcdMessageBox(UI_MSG_ERROR, UISTR_ENROLLFP_FPDOUBLED, 3000);
		}
		...
		{
			uiLcdMessageBox(UI_MSG_ERROR, UISTR_SERVER_NO_ENROLL_ID, UI_MSGTIMEOUT);
		}		
		else
		{
			uiLcdMessageBox(UI_MSG_ERROR, UISTR_VERIFY_GET_DATA_FAIL, UI_MSGTIMEOUT);
...
void  tcpclient_connect_Creatthread(void)
{
	...
	if(tcpclient_connect_thread_id)
		return ;
	comm_sendtime();
    xprintf("%s", "tcpclient_connect_Creatthread\r\n");	
...
u8 web2_glog_get_bytime(PACK_HEAD* vCMD, GetGLogCondition *pCMD)
{
	DWORD count;
	int SendCount;
	DWORD LogIndex[3],dwSeconds,ret;
	int pos;
	u32 loveywm = (u32)-1;
	count = DbGLogCountAll();
...
u8 web2_glog_get_byindex(PACK_HEAD* vCMD, GetGLogConditionEx *glogconditionex)
{
	...
	u32 loveywm = (u32)-1;
	int SendCount;
	ret = glogconditionex->EndIndex - glogconditionex->BeginIndex + 1;
	ALOG_INFO *pLogs = (ALOG_INFO*)malloc(sizeof(ALOG_INFO) * (ret));
	if(!pLogs)
	{
		//Com_SendExeResult( EXE_RES_FALSE, 0 );
...
u8 web2_cmd_get_log( PACK_HEAD* vCMD,u8 *recvdata)
{
	...
	{
		web2_glog_get_byall(vCMD);
	}
	else if(((vCMD->Datalen) == (1 + sizeof(GetGLogCondition))) && *recvdata == 2)
	{
		GetGLogCondition *glogcondition = NULL;
		glogcondition = (GetGLogCondition *)(recvdata + 1);
		web2_glog_get_bytime(vCMD, glogcondition);
	}
	else if(((vCMD->Datalen) == (1 + sizeof(GetGLogConditionEx))) && *recvdata == 3)
	{
		GetGLogConditionEx *glogconditionex = NULL;
		glogconditionex = (GetGLogConditionEx*)(recvdata + 1);
		web2_glog_get_byindex(vCMD, glogconditionex);
	}
	else
	{
		u32 loveywm = (u32)-1;
...
u8 web2_send_data(PACK_HEAD* vCMD,void *data,u32 datalen)
{
	...
	comm_tcpclient_send(sendbuf,PACK_HEAD_SIZE + len + 4);
	
	xprintf("send_data is ok111!!\r\n");
	free(sendbuf);
	xprintf("send_data is ok222!!\r\n");
	return TRUE;
}
...
u8 web2_send_finger(u64 ID,int backupnumber,u32 prilege,u8 *pdata)
{
   ...
   {	
	...
	if(backupnumber == BACKUP_PASSWORD)
	{
