...
#define INPUT_NUM_COUNT     17  //14
#define INPUT_NUM_COLS      6   //7
#define INPUT_NUM_SIZE_X    52
#define INPUT_NUM_SIZE_X1   128
#define INPUT_NUM_SIZE_Y    53
#define INPUT_NUM_START_X   24  //25
#define INPUT_NUM_START_Y   15  //30
#define INPUT_NUM_SPACE_X   24  //11
#define INPUT_NUM_SPACE_Y   65  //80
...
void uiTimeSetLastActionTime(DWORD dwTime/* = -1UL*/)
{
	...
	if (dwTime == -1UL)
		g_uiTimeLastAction = uiTimeGetTickCount();
	else
...
const int __KEYMAP_TEXTBOX[] = 
{
	...
	Qt::Key_Period,		    //UIKEY_PERIOD  
	Qt::Key_Colon,          //UIKEY_COLON   
	Qt::Key_Minus,          //UIKEY_MINUS   
	Qt::Key_Delete          //UIKEY_DELETE  
};
...
int uiTsGetInputNum(int bPostKeyEvent)     
{
	int nKey;
	int i, x, y, w, h, w_spacer, h_spacer;  
	
	uiGetTs();    

	nKey = UIKEY_NONE;

	w = INPUT_NUM_SIZE_X; h = INPUT_NUM_SIZE_Y;   
	x = INPUT_NUM_START_X; y = INPUT_NUM_START_Y; 
	w_spacer = INPUT_NUM_SPACE_X; 
	h_spacer = INPUT_NUM_SPACE_Y;

	for (i=0; i<INPUT_NUM_COUNT; i++)
	{
		if (i % INPUT_NUM_COLS == 0)
		{
			x = INPUT_NUM_START_X;
			y = y + h_spacer;
		}

		if (i == INPUT_NUM_COUNT-1)
            w = INPUT_NUM_SIZE_X1;
...
int uiTsGetInputABC(QWidget *pWindow, int bPostKeyEvent)     
{
	int nKey;
...
// 		if (nKey == Qt::Key_Tab)
// 		{
// 			QWidget *w_next = w->nextInFocusChain();
// 			if (w_next)
// 			{
// 				w_next->setFocus(Qt::MouseFocusReason);
// 				goto _lExit;
// 			}
// 		}
		...
		//	if (nKey == Qt::Key_Up && wCombo->currentIndex() == 0)
		//	{
		//		wCombo->setCurrentIndex(wCombo->count() - 1);
		//		goto _lExit;
		//	}
		}   
		...
		//	w = w->parentWidget();   
		 ...
		 if (ctrlType == CTRL_TEXTBOX && (nKey == Qt::Key_Delete) ) 
		{ 
			if (w)
			{
		    	QString strResult = ((QLineEdit*)w)->text();
				if (strResult.length() > 0)
				{
					strResult.remove(strResult.length() - 1, 1);
					((QLineEdit*)w)->setText(strResult);
				}   
				else ((QLineEdit*)w)->setText("");
				goto _lExit;
			}
		}

        QString szString = ((nKey >= Qt::Key_Minus && nKey <= Qt::Key_Colon) ? QString(nKey) : QString(0)); 
...
// 	}
	...
	if (nCount)
	{
		...
		strcpy(buf, "CPNo\t" "TMNo\t" "EnNo\t" "Name\t\t" "Tr\t" "Mode\t" "DateTime\r\n"); 
        ...
        for (i=0; i<nCount; i++)
		{
		//	uiLcdProgress(LCDPROG_PERCENT(i, nCount));
		//	if (!pLogBuffer[i].dwSeconds)
 		//		break;

		//	USER_INFO *pUserInfo = (USER_INFO*)DbUserInfoGetPointer(pLogBuffer[i].nID);
		    ...
		    sprintf(buf, "%d\t%d\t%d\t \t\t%d\t%d\t%s\r\n",       
				...
				//	" ",
				//	dbSetupTotal.setSystem.nMachineID,           //xf 091120
				    pLogBuffer[i].byTrStatus,                    //xf 091120
...
BOOL uiSL811_DownloadSlogInternal(const char* szFilePrefix, int nStartPos, BOOL* pbEmpty)  
{
	...
	sprintf(strFileName, "%s/%s_%03d.txt", USB_MOUNTPOINT, szFilePrefix, dbSetupTotal.setSystem.nMachineID);  
	f.setFileName(strFileName);
...
BOOL uiSL811_DownloadAllEnroll(void)
{
	...
	uiLcdProgressDlgShow(UISTR_MENU_USERINFODOWNLOAD); g_bProgress = TRUE;
...
BOOL uiEventSend_HEART(void)
{
    ...
    if( comm_send_event(EVENT_HEART, &dbLicense.szSerialNumber[0], 32 ) == 0 )	
	{
...
BOOL uiEventSend_GLOG(ALOG_INFO *pLogData)
{
	...
	if( comm_send_event(EVENT_GLOG, pLogData, sizeof(ALOG_INFO)) == 0 )
	{
		tcpclient_connect_start();
		bRet = FALSE;
	}
...
BOOL uiEventSend_SLOG(MLOG_INFO *pLogData)
{
	...
	if( comm_send_event(EVENT_SLOG, pLogData, sizeof(MLOG_INFO)) == 0 )
	{
		tcpclient_connect_start();
		bRet = FALSE;
	}
...
{
	xprintf("loveywm5\r\n");	
	//comm_send_event(EVENT_PRESS_FP, NULL, 0);
}
