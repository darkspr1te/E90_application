...
#define BELL_SCROLL_SIZE_Y         41   //21

#define MAX_FWCOUNT                6
#define FW_SCROLL_SIZE_Y           38

#define KEYBOARD_BACK_X   (480 - 320) / 2   
#define KEYBOARD_BACK_Y   (272 - 240) / 2   
#define KEYBOARD_ESC_X 	KEYBOARD_BACK_X + 120
#define KEYBOARD_ESC_Y 	KEYBOARD_BACK_Y + 170
#define KEYBOARD_ESC_W 	80
#define KEYBOARD_ESC_H 	40 

#define MESSAGE_BACK_X   (480 - 270) / 2   
#define MESSAGE_BACK_Y   (272 - 120) / 2 

#define SOUND_BACK_X   (480 - 320) / 2   
#define SOUND_BACK_Y   (272 - 240) / 2 
...
// 	__rowdata_bell[1].szValue = UNICODE2STRING(tz->NAME, TZNAME_LENGTH);
...
void CBellSetup::DoProcess()
{
    ...
    tblBellSet.SetTableRect(m_nFrameX, m_nFrameY, m_nFrameWidth, SCROLL_HEIGHT);
	tblBellSet.InsertColumn(-1, UISTR(UISTR_BELL_NO), Qt::AlignVCenter|Qt::AlignHCenter, 120);
	tblBellSet.InsertColumn(-1, UISTR(UISTR_BELL_STIME), Qt::AlignVCenter|Qt::AlignHCenter, 160);
	tblBellSet.InsertColumn(-1, UISTR(UISTR_BELL_LENGTH), Qt::AlignVCenter|Qt::AlignHCenter, 160);
	tblBellSet.SetRowDataCallback(GetRowData_bell);
	tblBellSet.SetRowHeight(BELL_SCROLL_SIZE_Y);
	...
	while (uiTimeIsTimeout() == FALSE)
	{
		...
		x = m_nFrameX+120; y = m_nFrameY + SCROLL_HDR_HEIGHT;  //x = m_nFrameX; 
		h = BELL_SCROLL_SIZE_Y;  w = 160;  //w = m_nFrameWidth;

		for (i=0; i<(MAX_BELLCOUNT/2); i++)
		{
			if ((nTouchX >= x) && (nTouchX <= (x+w)) && (nTouchY >= y) && (nTouchY <= (y+h)))  
			{
			//	nKey = UIKEY_OK;
				tz = &dbSetupTimezoneNew.tzBell[n_CurrentIndex + i];
		
				m_dwBellTime = DIALOGBOX_RETNUMBELL(CInputUserID);
				if (m_dwBellTime >= 0)
				{
            ...
            if ((nTouchX >= (x+w)) && (nTouchX <= (x+w+w)) && (nTouchY >= y) && (nTouchY <= (y+h)))  
			{
			//	nKey = UIKEY_OK;
				tz = &dbSetupTimezoneNew.tzBell[n_CurrentIndex + i];
		
				m_nBellLength = DIALOGBOX_RETNUM(CInputUserID);
				if (m_nBellLength >= 0 && (tz->RANGE.bStartHour != 0 || tz->RANGE.bStartMinute != 0)) 
				{
					tz->RANGE.bEndMinute = (BYTE)m_nBellLength;
				}
				break;
			}
		...
		case UIKEY_2:     
			tblBellSet.OnPageUp();
			n_CurrentIndex = tblBellSet.GetCurrentIndex();   
			break;
        case UIKEY_3:     
			tblBellSet.OnPageDown();
			n_CurrentIndex = (tblBellSet.GetCurrentIndex()+1) - (MAX_BELLCOUNT/2);   
			break;
		default:
...
//	int x, y, cx, cy;
    ...
    tblFiles.SetTableRect(m_nFrameX, m_nFrameY, m_nFrameWidth, SCROLL_HEIGHT);
	...
	tblFiles.SetRowHeight(FW_SCROLL_SIZE_Y);
	...
	while (uiTimeIsTimeout() == FALSE)
	{
        ...
        x = m_nFrameX; y = m_nFrameY;  
		h = FW_SCROLL_SIZE_Y;  w = m_nFrameWidth;

		for (i=0; i<MAX_FWCOUNT; i++)
		{
			if ((nTouchX >= x) && (nTouchX <= (x+w)) && (nTouchY >= y) && (nTouchY <= (y+h)))  
			{
				tblFiles.SetCurrentIndex(i);
                nKey = UIKEY_OK;
				break;
			}
...
int CSelftestKeyboard::DoProcess(DWORD dwTimeout /* = -1UL */)
{
	const char* __btn_names[] = 
	{
	    ...
	    "btnPERIOD", // UIKEY_PERIOD,
		"btnCOLON", // UIKEY_COLON,
		"btnMINUS", // UIKEY_MINUS,
		"btnDELETE", // UIKEY_DELETE,
		"btnF1", // UIKEY_F1,
		"btnF2", // UIKEY_F2,
		"btnF3", // UIKEY_F3,
		"btnF4", // UIKEY_F4,
	...
	int x_btnESC = KEYBOARD_BACK_X + ui.btnESC->x(), y_btnESC = KEYBOARD_BACK_Y + ui.btnESC->y();
    int w_btnESC = ui.btnESC->width(), h_btnESC = ui.btnESC->height();
	int nKey;
	QWidget *pCurrentWidget;

    ui.btnF1->setFocus(Qt::MouseFocusReason);  
	...
	while (uiTimeIsTimeout(dwTimeout) == FALSE)
	{
		...
		nKey = uiKeyGetKey();  
    	if ((nTouchX >= x_btnESC) && (nTouchX <= (x_btnESC+w_btnESC)) && (nTouchY >= y_btnESC) && (nTouchY <= (y_btnESC+h_btnESC)))  //xf 091120
		     break;
...
int CSelftestMemory::DoProcess()
{
	...
	BYTE bySave;
	BOOL bRet = TRUE;
	int x_btnESC = MESSAGE_BACK_X + ui.btnESC->x(), y_btnESC = MESSAGE_BACK_Y + ui.btnESC->y();
    int w_btnESC = ui.btnESC->width(), h_btnESC = ui.btnESC->height();
...
int CSelftestSound::DoProcess(DWORD dwTimeout /* = -1UL */)
{
	...
	int x_btnPLAY = SOUND_BACK_X + ui.btnPlay->x(), y_btnPLAY = SOUND_BACK_Y + ui.btnPlay->y();
    int w_btnPLAY = ui.btnPlay->width(), h_btnPLAY = ui.btnPlay->height();
	int x_btnESC = SOUND_BACK_X + ui.btnESC->x(), y_btnESC = SOUND_BACK_Y + ui.btnESC->y();
    int w_btnESC = ui.btnESC->width(), h_btnESC = ui.btnESC->height();
	int i;
...
// 		ui.comboBox->addItem(UISTR(i));
	...
	while (!m_bTerminateFlag && uiTimeIsTimeout(dwTimeout) == FALSE)
	{
		...
		nKey = uiKeyGetKey();
        if ((nTouchX >= x_btnPLAY) && (nTouchX <= (x_btnPLAY+w_btnPLAY)) && (nTouchY >= y_btnPLAY) && (nTouchY <= (y_btnPLAY+h_btnPLAY)))  
		{
			ui.comboBox->setCurrentIndex(0);
			uiSoundOut(sound_list[ui.comboBox->currentIndex()], UI_BUZZER_NONE);
			while (ui.comboBox->currentIndex()!= ui.comboBox->count()-1)
			{
				uiTimeDelay(2000);
