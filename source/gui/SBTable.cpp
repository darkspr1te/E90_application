...
#define SCROLL_WIDTH			(m_ScrollUse ? 20 : 0)  
#define SCROLL_UP_FILENAME	    "scroll-up.png"
#define SCROLL_DOWN_FILENAME	"scroll-down.png"
...
void SBTable::DrawScrollArea(QPainter *p)
{
    ...
    int y = m_tblArea.top() + LISTHDR_HEIGHT + LISTUP_HEIGHT;   
	int w = SCROLL_WIDTH;
    int h = m_tblArea.height() - LISTHDR_HEIGHT - LISTUP_HEIGHT * 2;  
	int h_scroll, h_scroll_1;
	int x_up = m_tblArea.left() + m_tblArea.width() - SCROLL_WIDTH;
    int y_up = m_tblArea.top() + LISTHDR_HEIGHT;
    int x_down = m_tblArea.left() + m_tblArea.width() - SCROLL_WIDTH;
    int y_down;

	QPixmap pmap_scrollBg(RESOURCE_PATH SCROLL_BG_FILENAME);
	QPixmap pmap_scrollItem(RESOURCE_PATH SCROLL_ITEM_FILENAME);
	QPixmap pmap_scrollUp(RESOURCE_PATH SCROLL_UP_FILENAME);
	QPixmap pmap_scrollDown(RESOURCE_PATH SCROLL_DOWN_FILENAME);
	...
	p->drawPixmap(x_up, y_up, LISTUP_HEIGHT, LISTUP_HEIGHT, pmap_scrollUp, 0, 0, pmap_scrollUp.width(), pmap_scrollUp.height());  //xf 091120
	p->drawPixmap(x_down, y_down, LISTUP_HEIGHT, LISTUP_HEIGHT, pmap_scrollDown, 0, 0, pmap_scrollDown.width(), pmap_scrollDown.height());  //xf 091120
