...
//////////////////////////////////////////////////////////////////////////
static BYTE g_byMACaddr_default[8] = {0x00, 0x13, 0x8F, 0x28, 0x47, 0xF2, 0x00, 0x0};
...
BOOL DbParamInfoLoad(void)
{
	...
	else //to Default
	{
		...
		g_params.ipAddress = UI_IP_MAKE(192, 168, 10, 201);
...
BOOL DbLicenseRead(void)
{
	...
	pby = (BYTE*)malloc(FLASH_LICENSE_SIZE); pdw = (DWORD*)pby;
	pbyBackup = (BYTE*)malloc(FLASH_LICENSE_SIZE);
	if (pby == NULL || pbyBackup == NULL)
	{
		bRet = FALSE;
...
// 			dwCheckSum++;
	...
	if (memcmp(dbLicense.byMACaddr, g_params.byMACaddr, 6))
	{
		memcpy(&dbLicense.byMACaddr[0], &g_params.byMACaddr[0], 6);
		DbLicenseWrite();
	}
...
//	*((WORD*)&pby[FLASH_LICENSE_SIZE - 2]) = CRC16Checksum(pby, FLASH_LICENSE_SIZE);
...
BOOL DbSetupTotalRead(BOOL bBackupCheck /*= TRUE*/)
{
	...
	if(fd == INVALID_HANDLE_VALUE)
		uiLcdMessageBox(UI_MSG_ERROR, "open SETUP.set ERR!", 3000);
	if (fd == INVALID_HANDLE_VALUE)
		return FALSE;
	
	pby = (BYTE*)malloc(FLASH_SETUP_SIZE); pdw = (DWORD*)pby;
	pbyBackup = (BYTE*)malloc(FLASH_SETUP_SIZE);
	if (pby == NULL || pbyBackup == NULL)
	{
		bRet = FALSE;
		uiLcdMessageBox(UI_MSG_ERROR, "malloc FLASH_SETUP_SIZE ram ERR!", 3000);
		goto _lExit;
	}
	...
	if (!bRet || (dwCheckSum != *(DWORD*)pby) || SetupTotaldata2Default_flag) //To Default
	{
		...
		//
		memset(&dbSetupTotal.setNoMenu.szCompanyNamy[0], 0, sizeof(dbSetupTotal.setNoMenu.szCompanyNamy));

		dbSetupTotal.setNoMenu.nReadMLogPos = 0;
		dbSetupTotal.setNoMenu.nReadALogPos = 0;
		if (bBackupCheck) //in the case of setup data is damaged.
		{
			dbSetupTotal.setNoMenu.dwMechanical = 0;
			dbSetupTotal.setNoMenu.dwExpose = 0;	//If this value is 0, use standard expose value.
													//If you upgrade the terminal of old version, this value will be unexpected,
													//therefore as soon as you must adjust CMOS brightness.
		}
		dbSetupTotal.setNoMenu.bDisableDevice = FALSE;
		dbSetupTotal.setNoMenu.byDoorStatus = 2;//Auto
		dbSetupTotal.setNoMenu.bSummertimeUpdated = FALSE;
		dbSetupTotal.setNoMenu.wiegand_type.byOEM_len = 0;
		dbSetupTotal.setNoMenu.wiegand_type.byFC_len = 8;
		dbSetupTotal.setNoMenu.wiegand_type.byCC_len = 16;
		dbSetupTotal.setNoMenu.wiegand_type.dwTpw = 100;
		dbSetupTotal.setNoMenu.wiegand_type.dwTpi = 900;

		//
		dbSetupTotal.setSystem.nTimeFormat = 0;
		dbSetupTotal.setSystem.nDateFormat = 0;
		dbSetupTotal.setSystem.nLanguage = UILANG_SC;
		dbSetupTotal.setSystem.nVolume = 50;
		dbSetupTotal.setSystem.bVoiceOut = TRUE;
		dbSetupTotal.setSystem.bKeyTones = TRUE;
		dbSetupTotal.setSystem.bPhotoShow = FALSE;
		dbSetupTotal.setSystem.bFPShow = TRUE;
		dbSetupTotal.setSystem.bCameraSave = TRUE;
		dbSetupTotal.setSystem.byCameraRateForFP = 0;
		dbSetupTotal.setSystem.byCameraRateForCard = 0;
		dbSetupTotal.setSystem.byCameraRateForPWD = 0;
		dbSetupTotal.setSystem.nDaylightOffset = 0;
		dbSetupTotal.setSystem.dwDaylightChangeTime = 0;
		dbSetupTotal.setSystem.dwDaylightRestoreTime = 0;
		dbSetupTotal.setSystem.byF1Status = TR_DUTYON;
		dbSetupTotal.setSystem.byF2Status = dbSetupTotal.setSystem.byF1Status + 1;
		dbSetupTotal.setSystem.byF3Status = dbSetupTotal.setSystem.byF1Status + 2;
		dbSetupTotal.setSystem.byF4Status = dbSetupTotal.setSystem.byF1Status + 3;
		dbSetupTotal.setSystem.byF5Status = dbSetupTotal.setSystem.byF1Status + 4;
		dbSetupTotal.setSystem.byF6Status = dbSetupTotal.setSystem.byF1Status + 5;
		dbSetupTotal.setSystem.byF7Status = dbSetupTotal.setSystem.byF1Status + 6;
		dbSetupTotal.setSystem.byF8Status = dbSetupTotal.setSystem.byF1Status + 7;
		dbSetupTotal.setSystem.nIdletimeForPoweroff = 0;
		dbSetupTotal.setSystem.nIdletimeForSleep = 0;
		dbSetupTotal.setSystem.nIdletimeForLcdswitch = 0;
		dbSetupTotal.setSystem.dwAutoPoweroffTime = 0;
		dbSetupTotal.setSystem.dwAutoPoweronTime = 0;
		dbSetupTotal.setSystem.dwAutoSleepTime = 0;
		dbSetupTotal.setSystem.bPowerkeyLock = FALSE;
		dbSetupTotal.setSystem.bRS232Use = TRUE;
		dbSetupTotal.setSystem.bRS485Use = TRUE;
		dbSetupTotal.setSystem.bUSBSlaveUse = TRUE;
		dbSetupTotal.setSystem.nMachineID = 1;
		dbSetupTotal.setSystem.nBaudrate = BAUDRATE_38400;  
		dbSetupTotal.setSystem.ipAddress = UI_IP_MAKE(192, 168, 10, 201);
		dbSetupTotal.setSystem.ipSubnetMask = UI_IP_MAKE(255, 255, 255, 0);
		dbSetupTotal.setSystem.ipDefaultGateway = UI_IP_MAKE(192, 168, 10, 1);
		dbSetupTotal.setSystem.ipPCBackground = UI_IP_MAKE(192, 168, 10, 101);
		dbSetupTotal.setSystem.wPortNumBack = 5005;  
		dbSetupTotal.setSystem.dwCommPassword = 0;
		dbSetupTotal.setSystem.nMLogWarning = 50;
		dbSetupTotal.setSystem.nGLogWarning = 500;
		dbSetupTotal.setSystem.nReverifyTime = 3;
		dbSetupTotal.setSystem.nLockTime = 5;
		dbSetupTotal.setSystem.nDoorOpenTime = 20;
		dbSetupTotal.setSystem.nDoorSensorStatus = DOORSENSOR_TYPE_OPEN;
		dbSetupTotal.setSystem.nIllegalVerifyWarning = 0;
		dbSetupTotal.setSystem.nManagersNumber = 5;
		...
		//dbSetupTotal.setSystem.bDHCPUse = TRUE;
		dbSetupTotal.setSystem.byAntipassStatus = ANTIPASS_NO;
		dbSetupTotal.setSystem.bPwdAlarmUse = FALSE;
		dbSetupTotal.setSystem.bSOSKeyUse = FALSE;
		dbSetupTotal.setSystem.nAlarmDelay = 0;
		dbSetupTotal.setSystem.nLockGroupMain = 1;
		dbSetupTotal.setSystem.dwBindingID = 0;
		dbSetupTotal.setSystem.bEthernetUse = TRUE;
		dbSetupTotal.setSystem.byNetworkSpeed = NETSPEED_100MBPS;
		dbSetupTotal.setSystem.byTheme = THEME_1;
		dbSetupTotal.setSystem.byWiegandFormat = WIEGAND_26;
		dbSetupTotal.setSystem.byDefaultGVM = VM_NONE;
		dbSetupTotal.setSystem.bCameraShow = FALSE;
		dbSetupTotal.setSystem.dwDataSaveTime = 30;
		dbSetupTotal.setSystem.bRemoteAttUse = TRUE;
		bzero(dbSetupTotal.setSystem.res, sizeof(dbSetupTotal.setSystem.res));

		dbSetupTotal.setSystem.nRealSpaceTime = 10;     
		dbSetupTotal.setSystem.DNSserverIp = UI_IP_MAKE(202, 96, 128, 86);	
		dbSetupTotal.setSystem.nBackgroupIconFile = 0;  

		memset(&dbSetupTotal.setSystem.szHostByName, 0, sizeof(dbSetupTotal.setSystem.szHostByName));
		
		//
		memset(&dbSetupTotal.setTimezone.tzTr[0], 0, sizeof(dbSetupTotal.setTimezone.tzTr));
		memset(&dbSetupTotal.setTimezone.tzBell[0], 0, sizeof(dbSetupTotal.setTimezone.tzBell));

		for (i=0; i<ACCESS_TIME_SET_COUNT; i++)
		{
			DB_TZACCESS *tz = &dbSetupTotal.setTimezone.tzAccess[i];
			for (k=0; k<7; k++)
			{
				tz->STATUS = 0;
				tz->RANGE[k].bStartHour = 0;
				tz->RANGE[k].bStartMinute = 0;
				tz->RANGE[k].bEndHour = 23;
				tz->RANGE[k].bEndMinute = 59;
			}
		}

		for (i=0; i<ACCESS_USER_GROUP_COUNT; i++)
		{
			DB_TZGROUP_A *tg = &dbSetupTotal.setTimezone.tzGroup[i];
			tg->bGroupA = 1;
			tg->bGroupB = 0;
			tg->bGroupC = 0;
			tg->bGroupD = 0;
		...
		memset(&dbSetupTotal.setTimezone.tzLockGroup[0], 0, sizeof(dbSetupTotal.setTimezone.tzLockGroup));
		dbSetupTotal.setTimezone.tzLockGroup[0] = NUMBER2LOCKGROUP(1);

		for (i=0; i<ACCESS_USER_GROUP_COUNT; i++)
			dbSetupTotal.setTimezone.tzGrupVM[i] = VM_NONE;

		memset(&dbSetupTotal.setTimezone.tzMsg[0], 0, sizeof(dbSetupTotal.setTimezone.tzMsg));

		//only for highlevel timezone function
		for (i=0; i<TIMEZONE_COUNT; i++)
		{
			DB_TIMESECTION_A *tm = &dbSetupTotal.setTimezone.tzHighTIME[i];
			tm->bStartHour = 0;
			tm->bStartMinute = 0;
			tm->bEndHour = 23;
			tm->bEndMinute = 59;
		}
		bzero(dbSetupTotal.setTimezone.tzHighTG, sizeof(dbSetupTotal.setTimezone.tzHighTG));
		bzero(dbSetupTotal.setTimezone.tzHighAccess, sizeof(dbSetupTotal.setTimezone.tzHighAccess));

		////////////////////////////////////////////////////////////////////////////////////////
// 		SB_EXEC("mv -f " ENROLLDATA_MOUNTPOINT "/*.set " "/tmp");
// 		SB_EXEC("rm -rf " ENROLLDATA_MOUNTPOINT "/*");
// 		SB_EXEC("mv -f " "/tmp/*.set " ENROLLDATA_MOUNTPOINT);
// 		if (SDCARD_MOUNT())
// 			SB_EXEC("rm -rf " SDCARD_MOUNTPOINT "/*");

//		DELETE_FILE(FLASH_LICENSE_FILENAME);
//		DELETE_FILE(FLASH_LICENSE_BACKUP);
//		DELETE_FILE(FLASH_SETUP_FILENAME);
//		DELETE_FILE(FLASH_SETUP_BACKUP);
//		DELETE_FILE(FLASH_UI_INDEX_FILENAME);
//		DELETE_FILE(FLASH_FP_INDEX_FILENAME);
//		DELETE_FILE(FLASH_ANTIPASS_FILENAME);
//		DELETE_FILE(FLASH_USERINFO_FILENAME);
//		DELETE_FILE(FLASH_FP_FILENAME);
//		DELETE_FILE(FLASH_SLOG_FILENAME);
//		DELETE_FILE(FLASH_GLOG_FILENAME);
//		DELETE_FILE(FLASH_PART_FILENAME);
		RMDIR(FLASH_MP3_CURRENT_DIRNAME);
		RMDIR(FLASH_SCREENSAVER_CURRENT_DIRNAME);
		////////////////////////////////////////////////////////////////////////////////////////

		szCurrentName = (char*)malloc(256);
		if (szCurrentName)
		{
			//MP3
			RMDIR(FLASH_MP3_CURRENT_DIRNAME);
			MKDIR(FLASH_MP3_CURRENT_DIRNAME);
			for (i=SOUND_START; i<=SOUND_END; i++)
			{
				sprintf(szCurrentName, "ln -s %s/%d.wav %s/%d.mp3", FLASH_MP3_BACKUP_DIRNAME, i, FLASH_MP3_CURRENT_DIRNAME, i);
				SB_EXEC(szCurrentName);
			}

			//Screensaver
			RMDIR(FLASH_SCREENSAVER_CURRENT_DIRNAME);
			MKDIR(FLASH_SCREENSAVER_CURRENT_DIRNAME);
			for (i=1; i<=FLASH_SCREENSAVER_IMAGE_COUNT; i++)
			{
				sprintf(szCurrentName, "ln -s %s/%d.jpg %s/%d.jpg", FLASH_SCREENSAVER_BACKUP_DIRNAME, i, FLASH_SCREENSAVER_CURRENT_DIRNAME, i);
				SB_EXEC(szCurrentName);
			}
			sprintf(szCurrentName, "ln -s %s/%s %s/%s",
				FLASH_SCREENSAVER_BACKUP_DIRNAME, FLASH_SCREENSAVER_MP3_FILENAME,
				FLASH_SCREENSAVER_CURRENT_DIRNAME, FLASH_SCREENSAVER_MP3_FILENAME);
			SB_EXEC(szCurrentName);
			FREE(szCurrentName);
		}
...
//		printf("Database Initialized\r\n");
	...
	{
		//>>RSI090114
		BOOL bCommParamSync = FALSE;
		bCommParamSync = bCommParamSync || (dbSetupTotal.setSystem.nMachineID != g_params.nMachineID);
		bCommParamSync = bCommParamSync || (dbSetupTotal.setSystem.nBaudrate != g_params.nBaudrate);
		bCommParamSync = bCommParamSync || (dbSetupTotal.setSystem.ipAddress != g_params.ipAddress);
		bCommParamSync = bCommParamSync || (dbSetupTotal.setSystem.ipSubnetMask != g_params.ipSubnetMask);
		bCommParamSync = bCommParamSync || (dbSetupTotal.setSystem.ipDefaultGateway != g_params.ipDefaultGateway);
		bCommParamSync = bCommParamSync || (dbSetupTotal.setSystem.dwCommPassword != g_params.dwCommPassword);
		if (bCommParamSync)
		{
			dbSetupTotal.setSystem.nMachineID = g_params.nMachineID;
			dbSetupTotal.setSystem.nBaudrate = g_params.nBaudrate;
			dbSetupTotal.setSystem.ipAddress = g_params.ipAddress;
			dbSetupTotal.setSystem.ipSubnetMask = g_params.ipSubnetMask;
			dbSetupTotal.setSystem.ipDefaultGateway = g_params.ipDefaultGateway;
			dbSetupTotal.setSystem.dwCommPassword = g_params.dwCommPassword;
			DbSetupTotalWrite();
		}
		//<<RSI090114

		SET_LANGUAGE(dbSetupTotal.setSystem.nLanguage);
		SET_GUITHEME((int)dbSetupTotal.setSystem.byTheme);

		dbComm_ipIsDHCP = dbSetupTotal.setSystem.bDHCPUse;
		memcpy(&dbComm_macAddress[0], &dbLicense.byMACaddr[0], 6);
		dbComm_machineID = dbSetupTotal.setSystem.nMachineID;
//		dbComm_wPortNum = dbSetupTotal.setSystem.wPortNum;
		if (dbComm_ipIsDHCP)
		{
			dbComm_ipAddress = dbComm_ipSubnetMask = dbComm_ipDefaultGateway = 0;  
			dbComm_ipPcBackground = dbSetupTotal.setSystem.ipPCBackground;  
		//	dbComm_dnsserverip=0;   //debug
		       dbComm_dnsserverip = dbSetupTotal.setSystem.DNSserverIp;   //debug
// 			xifconfig("eth0", dbComm_macAddress, &dbComm_ipAddress, &dbComm_ipSubnetMask, &dbComm_ipDefaultGateway, FALSE);
		...
		xifconfig_speed("eth0", (int)dbSetupTotal.setSystem.byNetworkSpeed);
...
//			LOCK_DOOR_CONTROL(__DOOR_STATUS[dbSetupTotal.setNoMenu.byDoorStatus]);
		...
		{
			LOCK_DEINIT();
			LockControl(__DOOR_STATUS[dbSetupTotal.setNoMenu.byDoorStatus]);
		}
	}
...
BOOL DbSetupTotalWrite(BOOL bChangeTheme /*= TRUE*/)
{
	...
	if (dbSetupTotal.setSystem.byWiegandFormat == WIEGAND_26)
	{
		dbSetupTotal.setNoMenu.wiegand_type.byOEM_len = 0;
		dbSetupTotal.setNoMenu.wiegand_type.byFC_len = 8;
		dbSetupTotal.setNoMenu.wiegand_type.byCC_len = 16;
	...
	{
		dbSetupTotal.setNoMenu.wiegand_type.byOEM_len = 0;
		dbSetupTotal.setNoMenu.wiegand_type.byFC_len = 16;
		dbSetupTotal.setNoMenu.wiegand_type.byCC_len = 16;
	}
	...
	*(DWORD*)pby = dwCheckSum;
	...
	bNetworkSetting = (dbComm_ipIsDHCP != dbSetupTotal.setSystem.bDHCPUse);
	if (!dbSetupTotal.setSystem.bDHCPUse)
	{
		...
		bNetworkSetting = 
			...
			(dbComm_ipSubnetMask != dbSetupTotal.setSystem.ipSubnetMask) ||
			(dbComm_ipDefaultGateway != dbSetupTotal.setSystem.ipDefaultGateway)||
			(dbComm_dnsserverip != dbSetupTotal.setSystem.DNSserverIp);
	}
	if (bNetworkSetting)
	{
		dbComm_ipIsDHCP = dbSetupTotal.setSystem.bDHCPUse;
		...
		//	DnsIpSet(dbSetupTotal.setSystem.DNSserverIp); 
		}
		else
		{   
			dbComm_ipAddress = dbSetupTotal.setSystem.ipAddress;
...
// 	if (dbComm_wPortNum != dbSetupTotal.setComm.wPortNum)
// 	{
// 		dbComm_wPortNum = dbSetupTotal.setComm.wPortNum;
// 		comm_tcpserver_close();
// 		comm_tcpserver_init();
// 	}
...
//		g_params.wPortNum = dbSetupTotal.setSystem.wPortNum;
		DbParamInfoSave();
	}
_lExit:
	if (pby)
...
BOOL DbUserInfoEnroll(USER_INFO *pUserInfoNew, int nBackup, void* pData, BOOL bFlashWrite /*= TRUE*/, BOOL bNoMatch /*= TRUE*/)
{
	...
	if (nBackup >= BACKUP_FINGER0 && nBackup <= BACKUP_FINGER9)
	{
		if (!DbFpSetPointer(nPos, nBackup, pData, bNoMatch, bFlashWrite))
		{
			bRet = FALSE;
			goto _lFail;
		}
...
{
	if (/*!SDCARD_MOUNT() || */!DbUserInfoFlagVoiceStatusGet(pUserInfo))
		return FALSE;

	char *szFilename = MAKE_FULLPATH(SDCARD_MOUNTPOINT PATH_USER_VOICE, ID2STRING(pUserInfo->ID), NULL);
...
{
	char *szFilename = MAKE_FULLPATH(SDCARD_MOUNTPOINT PATH_USER_VOICE, ID2STRING(nID), NULL);
	sprintf(&szFilename[strlen(szFilename)], "voice.wav");
	return szFilename;
}
...
{
	if (/*!SDCARD_MOUNT() || */!DbUserInfoFlagPhotoStatusGet(pUserInfo))
		return FALSE;
	return FILE_EXIST(MAKE_FULLPATH(SDCARD_MOUNTPOINT PATH_USER_PHOTO, ID2STRING(pUserInfo->ID), "jpg"));
}
...
{
	return MAKE_FULLPATH(SDCARD_MOUNTPOINT PATH_USER_PHOTO, ID2STRING(nID), "jpg");
}
...
//////////////////////////////////////////////////////////////////////////
int DbUserInfoGetUsedIDPositon(UINT64 nID)
{
	int i;
...
// 	}
	...
	{
		int from, to, step;
		DWORD dwAntipassLastDate = DbAntipassGetLastDate();
		DWORD dwSecond = uiRtcGetSeconds();
...
//////////////////////////////////////////////////////////////////////////
int	DbGLogGetByTime(ALOG_INFO *pLogBuffer,int from,int to, int fromsecond, int tosecond)
{
	int fd;
	//int nResult, nResult1;
	int nCount;
...
//////////////////////////////////////////////////////////////////////////
int	DbGLogPosGetByTime(int from,int to, int fromsecond, int tosecond)
{
	int fd;
    int i;
	ALOG_INFO logtemp;
...
//////////////////////////////////////////////////////////////////////////
int	DbGLogCountGetByTime(int from,int to, int fromsecond, int tosecond)
{
	int fd;
    int i;
	ALOG_INFO logtemp;
...
//	FlashWriteWithBackupPolling(TRUE);
	...
	if (!__DB_FIST_LOAD)
	{
		uiLcdProgressDlgShow(UISTR_DB_LOAD);
		g_bProgress = TRUE;
	}
...
DWORD	DbGLogCountGetByTimeAllSearch_and_byid( int fromsecond, int tosecond,u64 id)
{
	int fd;
      DWORD i;
	ALOG_INFO logtemp;
...
int	DbGLogGetByTime_and_byID(ALOG_INFO *pLogBuffer,int from,int to, int fromsecond, int tosecond,u64 id)
{
	int fd;
	//int nResult, nResult1;
	int nCount;
