...
void InitOther(void)
{
	...
	//ioctl(g_hOther, IOCTL_LED_ON, 0);    // µ„¡¡À¸
	//close(g_hOther);
}
...
static BOOL InitTS(void)
{
	g_hTSDevice = open(DEVNAME_TS, O_RDWR);
	if (g_hTSDevice == -1)
	{
		printf("TouchScreen Device open error !\r\n");
...
//	DEVINIT_WRAPPER(InitSDMod(),        DEVERR_SDMOD);
	DEVINIT_WRAPPER(InitCAMMod(),		DEVERR_CAMMOD);
	DEVINIT_WRAPPER(InitPort(),			DEVERR_GPIO);
	DEVINIT_WRAPPER(InitSBUM(),			DEVERR_SBUM);
    	DEVINIT_WRAPPER(InitTS(),			DEVERR_TS);  
//	DEVINIT_WRAPPER(InitServerSocket(),	DEVERR_SUCCESS);
        //DEVINIT_WRAPPER(InitCameraDevice(), DEVERR_CAMERA);
//	DEVINIT_WRAPPER(InitFpLibrary(),	DEVERR_ENGINE0);
...
BOOL InitCameraDevice(void)
{
	...
	if (sb_init_videoIn(&g_VideoIn, 480, 272, VIDEO_PALETTE_JPEG) != 0)  
	{
...
void GETTS(void)
{
	...
	if (g_hTSDevice != INVALID_HANDLE_VALUE)
		read(g_hTSDevice, &ts_ret, sizeof(TS_RET));
}
...
void SETTS(void)
{
	memset(&ts_ret, 0, sizeof(TS_RET));
	if (g_hTSDevice != INVALID_HANDLE_VALUE)
		write(g_hTSDevice, &ts_ret, sizeof(TS_RET));
}
...
void uiGetTs(void) 
{
	...
	nTouchPress = 0;
		...
		if((ts_ret.x >= TOUCH_MIN_X ) && (ts_ret.y >= TOUCH_MIN_Y ) && (ts_ret.pressure == PEN_DOWN))
		{
		       LCD_CTL_ON();
			g_uiProcStatus.dwLcdBackTime = uiTimeGetTickCount();
					   
			uiBuzzerOn(UI_BUZZER_ALARM);
			...
			nTouchY = LCD_SIZE_Y - nTouchY;   
			nTouchPress = PEN_DOWN;  
		}
}
...
BYTE GETKEY(BOOL bLong)
{
	...
	if ( KEYCODE != -1 &&  KEYCODE != UIKEY_NONE )
	{
		  LCD_CTL_ON();
		  g_uiProcStatus.dwLcdBackTime = uiTimeGetTickCount();
	}
...
#define SOUND_STOP()	{GPIO_SET_VALUE(GPIO_STG, 0, 2, 0);GPIO_SET_VALUE(GPIO_STG, 3, 2, 0);}  

//////////////////////////////////////////////////////////////////////////
void APLUS_SOUND_BELL()
{
	SOUND_STOP();
    GPIO_SET_VALUE(GPIO_STG, 1, 1, 1);  
}
...
{
       DWORD dwNUM[IDNUMBER_LENGTH]={1,10,100,1000,10000,100000,1000000,10000000,100000000};
...
//////////////////////////////////////////////////////////////////////////
int STRING2NUM(char* strNum)
{
	int nLength, i;
	int nNum = 0;

	if ((nLength = strlen(strNum)) > 8)
		return 0;

	for (i=0; i<nLength; i++)
	{
		if (strNum[i] >= '0' && strNum[i] <= '9')
			nNum = nNum * 10 + (strNum[i] - '0');
	}
...
//////////////////////////////////////////////////////////////////////////
DWORD STRING2NUMIP(char* strNum)
{
	int nLength, i, j;
	DWORD dwNum = 0;
	int addr[4];
	...
	for (j=0; j<4; j++)
    {
		while ( strNum[i] != '.' && i < nLength )
		{
			if (strNum[i] >= '0' && strNum[i] <= '9')
				addr[j] = addr[j] * 10 + (strNum[i] - '0');
			i++;
		}
		if ( strNum[i] == '.' )nstate++;  
		i++;
	...
	if( nstate >= 3 )
		dwNum = UI_IP_MAKE(addr[0], addr[1], addr[2], addr[3]);  

	return dwNum;
}

//////////////////////////////////////////////////////////////////////////
DWORD STRING2NUMDATE(char* strNum)
{
	int nLength, i, j;
	DWORD dwNum = 0;
	int addr[3];
	...
	{   	
		...
		if (nYear > nMax[0])
			nYear = nMax[0];
		else if (nYear < nMin[0])
			nYear = nMin[0];

        if (nMonth > nMax[1])
			nMonth = nMax[1];
		else if (nMonth < nMin[1])
			nMonth = nMin[1];
        ...
        dwNum = uiRtcGetSeconds(nYear, nMonth, nDay, 0, 0, 0);
	}
...
//////////////////////////////////////////////////////////////////////////
DWORD STRING2NUMTIME(char* strNum)
{
	int nLength, i, j;
	DWORD dwNum = 0;
	int addr[3];
	...
	if( nstate >= 2 )
	{
		for (i=0; i<3; i++)
		{
			if (addr[i] > nMax[i])
				addr[i] = nMax[i];
			else if (addr[i] < nMin[i])
				addr[i] = nMin[i];
		}

		uiRtcSeconds2YMDHMS(uiRtcGetSeconds(), &nYear, &nMonth, &nDay, &nHour, &nMinute, &nSecond);
        dwNum = uiRtcGetSeconds(nYear, nMonth, nDay, addr[0], addr[1], addr[2]);
	}
...
//////////////////////////////////////////////////////////////////////////
DWORD STRING2NUMBELL(char* strNum)
{
	int nLength, i, j;
	DWORD dwNum = 0;
	int addr[2];
	...
	if( nstate >= 1 )
	{
		...
		uiRtcSeconds2YMDHMS(uiRtcGetSeconds(), &nYear, &nMonth, &nDay, &nHour, &nMinute, &nSecond);
        dwNum = uiRtcGetSeconds(nYear, nMonth, nDay, addr[0], addr[1], nSecond);
	}
...
//////////////////////////////////////////////////////////////////////////
DWORD STRING2NUMDAYLIGHT(char* strNum)
{
	int nLength, i, j;
	DWORD dwNum = 0;
	int addr[4];
	...
	int nYear, nMonth, nDay, nHour, nMinute, nSecond;
	int nMin[] = {1,  1, 0, 0};
	int nMax[] = {12, 31, 23, 59};

	if ((nLength = strlen(strNum)) > 12)
		return 0;
	...
	for (j=0; j<4; j++)
    {
		while ( strNum[i] != '-' && strNum[i] != '.' && strNum[i] != ':' && i < nLength )
		{
			if (strNum[i] >= '0' && strNum[i] <= '9')
				addr[j] = addr[j] * 10 + (strNum[i] - '0');
			i++;
		}
		if ( (strNum[i] == '-') || (strNum[i] == '.') || (strNum[i] == ':') )nstate++;
		i++;
	}
	...
	{   	
		...
		uiRtcSeconds2YMDHMS(uiRtcGetSeconds(), &nYear, &nMonth, &nDay, &nHour, &nMinute, &nSecond);
     // nYear=2010; addr[0]=5; addr[1]=5; addr[2]=12; addr[3]=0; nSecond=0;
        dwNum = uiRtcGetSeconds(nYear-2000, addr[0], addr[1]+1, addr[2], addr[3], nSecond);
	}
